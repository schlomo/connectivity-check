...
def check_speed_ookla(self, target: int = -1, latency: int = 50, download: int = 20, upload: int = 5):
    """
    ...
    if target == 0:
        print("Set the TARGET to a speedtest.net server ID. Here are the closest servers:")
        return s.get_closest_servers()
    elif target > 0:
        try:
            server_list = list(s.get_servers([target]).values())[
                0]  # one in - one out
        except speedtest.NoMatchedServers:
            raise ConnectivityCheckException(
                f"Speedtest server {target} is invalid")
    else:  # target < 0
    ...
    result_server_description = "{id} ({host} by {sponsor} in {name}, {country})".format(
        **s.results.server)
    check_result = result_download >= download and result_upload >= upload and result_latency <= latency
    self._datadog(target=target, check="speed_ookla",
